...
class DoctorAppointmentAgent:
    ...
    def supervisor_node(self, state: AgentState) -> Command[Literal['information_node', 'booking_node', '__end__']]:
        ...
        print("messages___", messages)
        print("len(state['messages'])______", len(state['messages']))
        
        # query = state['messages'][-1].content if state["messages"] else ""
        query = ''
        if len(state['messages']) > 1:
            print("state['messages'][0].content_________", state['messages'][0])
